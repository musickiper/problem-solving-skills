...
public class RearrangeArray {
  ...
  public static char[] rearrange(char[] origin) {
    ...
    while (true) {
      // [index of max frequency, index of 2th Max frequency, index of min
      // frequency]
      int[] indexes = findMaxAndSecMaxAndMin(values);
      if (values.get(indexes[0]).equals(values.get(indexes[1]))
          && values.get(indexes[1]).equals(values.get(indexes[2]))) {
        // When all the characters has the same frequency, break while loop
        break;
      } else {
  ...
  private static int[] findMaxAndSecMaxAndMin(List<Integer> numbers) {
    int max = 0;
    int secMax = 0;
    int min = 0;
